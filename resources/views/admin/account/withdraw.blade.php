...
<form action="{{ route('admin.account-trans.withdraw_update', $model->id) }}" method="post" id="modal_form">
    @csrf
    @method('PATCH')
    <div class="row">
        {{-- Cash/Bank By A/c(Debit) --}}
        <div class="col-md-4 form-group">
            <label for="bank">Withdraw A/c <span
        ...
        <div class="col-md-6 form-group mx-auto" >
            <label for="amount">Withdraw Amount <span
                    class="text-danger">*</span></label>
            <input autocomplete="off" value="{{$model->amount}}" type="text" name="amount" required id="amount" class="form-control input_number" placeholder="Enter Withdraw Amount">
        </div>
