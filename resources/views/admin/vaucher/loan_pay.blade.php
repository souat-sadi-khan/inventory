...
<form action="{{ route('admin.vaucher.loan_pay_store') }}" method="post" id="modal_form">
    @csrf
    <div class="row">
        ...
        <div class="col-md-4 form-group">
            <label for="loan_pay">Loan Account A/c <span
                    class="text-danger">*</span></label>
            <select name="loan_pay" id="loan_pay" data-url="{{ route('admin.vaucher.balance_check') }}" required class="form-control select" data-placeholder="Select Account">
                <option value="">Select Account</option>
                @foreach ($loan as $model)
                    <option  value="{{ $model->id}}">{{ $model->name }} ({{ toWord($model->category) }})</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-2 form-group">
            <label for="loan_pay_amount">.</label>
            <input type="text" name="loan_pay_amount" value="00.00" readonly  id="loan_pay_amount" class="form-control" placeholder="Enter Payment Amount">
        </div>
        ...
        <div class="col-md-6 form-group mx-auto" >
            <label for="amount">Payment Amount <span
                    class="text-danger">*</span></label>
            <input type="number" name="amount" required id="amount" class="form-control" placeholder="Enter Payment Amount">
        </div>
...
<script>
      ...
      $(document).on('change', '#loan_pay', function () {
        var value = $(this).val();
        var url = $(this).data('url');
