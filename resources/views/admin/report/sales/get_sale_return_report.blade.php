<div id="print_table" class="card-body">
    ...
    <div class="text-center col-md-12">
        <h4 style="margin:0px ; margin-top: 7px; border:solid 1px #000; border-radius:50px; display:inline-block; padding:10px;"
            ...
            <b>@lang('Sale Return Report')</b></h4>
    </div>
    <br>
        ...
        <table class="table table-hover table-bordered content_managment_table table-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><a data-url ="{{ route('admin.sale_voucher.return.show',$element->return_parent_id) }}" id="btn_modal" style="cursor: pointer;">{{ $element->reference_no }}</a></td>
                    <td>{{ $element->date }}</td>
                    <td>{{ $element->discount_amount }}</td>
                    <td>{{ get_option('currency_symbol') }} {{ number_format($element->net_total, 2) }}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
