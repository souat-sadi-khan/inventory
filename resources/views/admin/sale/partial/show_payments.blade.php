@if($transaction->net_total - $transaction->payment()->sum('amount') > 0)
<h3>@lang('Make Payment')</h3>
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.post_sale_payment') }}"  method="post" id="modal_form">
            ...
            <div class="row">
                  <div class="col-md-12">
                    ...
                    <input type="text" class="form-control" name="payment_ref_no" value="{{ random_num('SPayment') }}" readonly>
                </div>
                <div class="col-md-12">
                    <label for="payment_date">
                        @lang('Date')
        ...
        <table class="table bg-gray">
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $pay->method }}</td>
                    <td>{{ $pay->note }}</td>
                    <td><button class="btn btn-sm btn-primary" onclick="myFunction('{{ route("admin.sale_voucher.sale.printpayment",$pay->id) }}')"><i class="fa fa-print" aria-hidden="true"></i></button></td>
                </tr>
                @endforeach
            </tbody>
        </table>
