@extends('layouts.main', ['title' => $sale_type=='retail'?('Retail Sale'):('Wholesale'), 'modal' => 'xl',])
@push('admin.css')
<style>
.bg-gray {
color: #000 !important;
...
@section('header')
 <a href="{{ route('admin.general.settings') }}" class="btn btn-light border-right ml-2 py-0"><i class="fa fa-cog fa-spin" aria-hidden="true"></i>@lang('Setting')</a>
    ...
    <a href="{{ route('admin.sale_voucher.sale.index') }}" class="btn btn-light border-right ml-2 py-0"><i class="fa fa-bath" aria-hidden="true"></i>@lang('Sale List')</a>
    @if ($sale_type=='retail')
...
<div class="col-md-12">
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Create {{ $sale_type=='retail'?'Retail Sale':'Wholesale' }} </h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <form action="{{ route('admin.sale_voucher.sale.store') }}" method="post" id="content_form">
                ...
                <div class="row">
                       <div class="col-md-5 form-group">
                            ...
                            <div class="input-group"  data-target-input="nearest">
                                <select name="customer_id" data-placeholder="Select Customer" id="customer_id" class="form-control" >
                                    @foreach ($customers as $element)
                                        <option value="{{ $element->id }}">{{ $element->customer_name }}({{ $element->customer_mobile }})</option>
                                    @endforeach
                                </select>
                                <div data-url="{{ route('admin.quick_customer') }}" title="Add New Customer" class="input-group-append btn_modal" style="cursor: pointer;">
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="text" name="invoice_no" id="invoice_no" class="form-control" value="{{$sale_type=='retail'?random_num('Retail'):random_num('Wholesale') }}" readonly>
                    </div>
                    <div class="col-md-5">
                ...
                <h3 class="bg-info text-center mt-4">
                Sale Product
                </h3>
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <table class="table table-bordered">
                            <thead class="bg-gray">
                                <tr>
                                    ...
                                    <td>
                                        <label for="product_qty">Sale Qty</label>
                                        <input type="text" name="product_qty" id="product_qty" class="form-control">
                                        <input type="hidden" id="availbale_qty" class="form-control input_number">
                                    ...
                                    <td>
                                        ...
                                        <input type="hidden" name="product_cost_price" id="product_cost_price" class="form-control input_number">
                                    </td>
                                </tr>
                            </thead>
                        </table>
                ...
                <div class="row mt-2">
                    <div class="col-md-12">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody class="bg-gray" id="saleDetailsRender">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <input type="hidden" name="purchase_value_sub_total" class="purchase_value_sub_total">
                    ...
                    <div class="col-md-11 mx-auto">
                        <div class="card" style="">
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <label for="transaction_note">Sale Note </label>
                                        <textarea name="transaction_note" class="form-control" id="transaction_note" placeholder="Sale Note"></textarea>
                                    </div>
                                    <div class="col-md-6">
...
$('#customer_id').select2({width:'90%'});
$(document).on('change', '#product', function() {
    var product = $('#product').val();
    $.ajax({
            url: '/admin/sale-voucher/product',
            data: {
                product: product,
            },
            type: 'Get',
        ...
        .done(function(data) {
            $('#availbale_qty').val(data.stock);
            $('#product_price').val(data.product_price);
            $('#product_cost_price').val(data.product_cost);
            $("#qty_error").text('');
        })
});
...
$(document).on('click', '#click_here', function() {
    ...
    if (product == "" || quantity == "") {
        ...
        $.ajax({
            url: '/admin/sale-voucher/add-product',
            data: {
                product: product,
                quantity: quantity,
...
$(document).on('keyup','#product_qty',function(){
        var qty =$(this);
        var able =$('input#availbale_qty').val();
             if (isNaN(qty.val())) {
                ...
                if ((qty.val() -0) > ($('input#availbale_qty').val()-0)) {
                  toastr.error('Sorry! this much not quantity is not available! available Qty is :'+' '+able);
                  $('#qty_error').text('quantity is not available!available Qty is :'+' '+able)
                  qty.val(1);
                }
        }
...
$("#saleDetailsRender").on('click', '.remmove', function() {
...
function calculate() {
    var sub_total = 0;
    var purchase_value_sub_total = 0;
    ...
    $(".cost_amt").each(function() {
        purchase_value_sub_total = purchase_value_sub_total + ($(this).html() * 1);
    })
    ...
    $(".purchase_value_sub_total").val(purchase_value_sub_total);
